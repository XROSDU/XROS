...
impl XROSLauncher {
    pub fn new(fetch_node_info: bool) -> Self {
        ...
        Self {
            ...
            data: Arc::new(XROSData::default()),
        }
    }

    async fn handle_xros_event(event: GlobalCtxEvent, data: &XROSData) {
        ...
        tasks.spawn(async move {
            ...
            loop {
                match receiver.recv().await {
                    Ok(event) => {
                        Self::handle_xros_event(event.clone(), &data_c).await;
                    }
                ...
                loop {
                    ...
                    *data_c.tun_dev_name.write().unwrap() =
                    ...
                    let node_info = MyNodeInfo {
                        ...
                        version: XROS_VERSION.to_string(),
        ...
        F: FnOnce() -> Result<TomlConfigLoader, anyhow::Error> + Send + Sync,
    {
        ...
        self.thread_handle = Some(std::thread::spawn(move || {
            ...
            let ret = rt.block_on(Self::xros_routine(
                cfg,
                stop_notifier.clone(),
                data,
                fetch_node_info,
...
impl NetworkInstance {
    ...
    pub fn is_xros_running(&self) -> bool {
    ...
    pub fn start(&mut self) -> Result<EventBusSubscriber, anyhow::Error> {
        if self.is_xros_running() {
            return Ok(self.subscribe_event().unwrap());
        }

        let launcher = XROSLauncher::new(self.fetch_node_info);
