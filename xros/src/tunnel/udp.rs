...
async fn respond_stun_packet(
    ...
    // we only respond xros stun req, whose tid has 0xdeadbeef prefix
    if tid.as_bytes()[0..4] != [0xde, 0xad, 0xbe, 0xef] {
        anyhow::bail!("stun req tid not from xros");
    }
...
mod tests {
    ...
    async fn ipv6_domain_pingpong() {
        ...
        let mut connector =
            UdpTunnelConnector::new("udp://test.xros.top:31016".parse().unwrap());
