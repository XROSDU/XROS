...
pub fn init_logger(
    ...
    if file_level != LevelFilter::OFF || need_reload {
        ...
        let file_appender = tracing_appender::rolling::Builder::new()
            ...
            .max_log_files(5)
            .filename_prefix(file_config.file.unwrap_or("xros".to_string()))
            .filename_suffix("log")
...
pub fn setup_panic_handler() {
    ...
    std::panic::set_hook(Box::new(|info| {
        ...
        let tmp_path = std::env::temp_dir().join("xros-panic.log");
        let candidate_path = vec![
            std::path::PathBuf::from_str("xros-panic.log").ok(),
            Some(tmp_path),
        ];
        let mut file = None;
        ...
        write_err(format!("panic occurred, if this is a bug, please report this issue on github (https://github.com/xros/xros/issues)"));
        write_err(format!("xros version: {}", crate::VERSION));
