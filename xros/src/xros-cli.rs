...
#[derive(Parser, Debug)]
#[command(name = "xros-cli", author, version = XROS_VERSION, about, long_about = None)]
struct Cli {
    /// the instance name
...
enum SubCommand {
    ...
    VpnPortal,
    #[command(about = "inspect self xros-core status")]
    Node(NodeArgs),
    #[command(about = "manage xros-core as a system service")]
    Service(ServiceArgs),
...
enum ServiceSubCommand {
    #[command(about = "register xros-core as a system service")]
    Install(InstallArgs),
    #[command(about = "unregister xros-core system service")]
    Uninstall,
    #[command(about = "check xros-core system service status")]
    Status,
    #[command(about = "start xros-core system service")]
    Start,
    #[command(about = "stop xros-core system service")]
    Stop,
}
...
struct InstallArgs {
    ...
    #[arg(long, help = "path to xros-core binary")]
    ...
    #[arg(
        ...
        help = "args to pass to xros-core"
    )]
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: xros::proto::common::Ipv4Inet| ip.address.unwrap_or_default())
                        .map(|ip| ip.to_string())
                        .unwrap_or_default(),
            ...
            match service_args.sub_command {
                ServiceSubCommand::Install(install_args) => {
                    ...
                    let bin_path = std::fs::canonicalize(bin_path).map_err(|e| {
                        anyhow::anyhow!("failed to get xros core application: {}", e)
                    })?;
...
mod win_service_manager {
    ...
    use xros::common::constants::WIN_SERVICE_WORK_DIR_REG_KEY;
