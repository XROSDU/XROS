...
#[derive(Parser, Debug)]
#[command(name = "xros-core", author, version = XROS_VERSION , about, long_about = None)]
struct Cli {
    #[arg(
        short = 'w',
        long,
    ...
    fn try_from(cli: &Cli) -> Result<Self, Self::Error> {
        ...
        if cli.file_log_dir.is_some() || cli.file_log_level.is_some() {
            ...
            cfg.set_file_logger_config(FileLoggerConfig {
                ...
                file: Some(format!("xros-{}", inst_name)),
            });
        }
...
async fn run_main(cli: Cli) -> anyhow::Result<()> {
    ...
    if cli.config_server.is_some() {
        ...
        let config_server_url = match url::Url::parse(&config_server_url_s) {
            Ok(u) => u,
            Err(_) => format!(
                "udp://config-server.xros.cn:22020/{}",
                config_server_url_s
            )
            .parse()
            .unwrap(),
        ...
        println!("Official config website: https://xros.cn/web");
    ...
    println!("Starting xros with config:");
    println!("############### TOML ###############\n");
    println!("{}", cfg.dump());
    println!("-----------------------------------");
